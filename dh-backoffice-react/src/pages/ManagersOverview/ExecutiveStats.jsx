...
import { Users, CheckSquare, Crown } from 'lucide-react';

/**
 * 📊 ส่วนแสดงสถิติภาพรวม (Executive Stats)
 * รับ Props ข้อมูล `stats` มาจาก useManagerDashboard
 */
const ExecutiveStats = ({ stats, onOpenStaffModal, onOpenVipModal, onNavigateTodo }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-baseline gap-2">
            <h3 className="text-3xl font-black text-[var(--dh-text-main)]">{stats.pendingStaffCount}</h3>
      ...
      <div 
        ...
        className="bg-[var(--dh-bg-surface)] p-5 rounded-2xl border border-rose-200 shadow-sm flex items-center gap-4 cursor-pointer hover:shadow-md transition-all group"
      >
        ...
        <div>
          <p className="text-[12px] font-bold text-[var(--dh-text-muted)] uppercase tracking-wider mb-1">งานรออนุมัติ (Todo)</p>
          <div className="flex items-baseline gap-2">
            <h3 className="text-3xl font-black text-[var(--dh-text-main)]">{stats.pendingTasksCount}</h3>
            <span className="text-sm font-medium text-rose-500">รายการ</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-baseline gap-2">
            <h3 className="text-3xl font-black text-[var(--dh-text-main)]">{stats.vipCount}</h3>
