...
**Note (CRITICAL):** All approval requests (including Ads which formerly used a duplicate `manager_todos` collection) must be routed to this root `todos` collection ONLY. 
**Do NOT write duplicate tasks to `manager_todos` or `artifacts/.../manager_todos`.** This prevents data redundancy, saves Firebase write costs, and ensures proper sync with the Central Todo board.
...
| `status`       | String    | Current status of the task.                                                 | `"pending_manager"`, `"approved"`, `"waiting_item"`, `"processing"`, `"completed"`, `"rejected"`, `"cancelled"` |
...
| `defectQuantity` | Number  | Stock quantity for broken items pending return to supplier (Claim module).  | `2`                                 |
...
The Calendar module operates without storing data locally in Firestore to conserve quota. It communicates directly with Google Apps Script (GAS).

### Architectural Note:
- **`calendarService.js`**: Connects the React front-end to GAS via the `fetchFromGAS` utility.
- All Read/Write (e.g., `createCalendarEvent`) operations interact with the user's connected Google Account directly.
- The system logs the action via `historyService.addLog()` asynchronously after successfully communicating with GAS.

---

## 21. Big Seller Export Integration (Quota Optimization)

To prevent massive Firestore Read spikes when exporting inventory data for Shopee/Lazada (via Big Seller), the system uses a **Zero-Quota Export Architecture**.

### Architectural Note:
- **`bigSellerExportService.js`**: Orchestrates the export process without touching Firestore `products`.
- **Process Flow**:
